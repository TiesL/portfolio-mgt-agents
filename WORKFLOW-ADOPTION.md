...
| process-context-document | yes | 2026-09-19 | Domain has real jargon (PID, PSA, DEC-XXX, Portfolio Board vs. Steering Group, governance tiers) not self-evident to a newcomer; cheap to grow incrementally rather than complete upfront. Confirmed with TiesL. |
| process-issue-tracking | yes | 2026-09-19 | Already practiced (issue-per-change, PR-per-issue) but issues #1/#3/#5 didn't follow the required Epic/Work-item structure (AC<n> Given/When/Then, `Covers:` field) — this answer commits to fixing that starting with #5, not retrofitting past issues. Confirmed with TiesL. |
| test-integration | no | 2026-09-19 | No code and no external dependency exists yet to integration-test. Trigger to revisit: once an implementation choice introduces a real external dependency (e.g. an LLM API call, a document/data store). Confirmed with TiesL. |
...
| spec-compliance | yes | 2026-09-19 | Substantiated in `PRD.md` — DEC-001 (human Go/No-Go) plus an intended audit trail is process-level substantiation; no legal/regulatory retention obligation identified yet, left to adopting organizations. Confirmed with TiesL. |
...
| spec-usability | yes | 2026-09-19 | Substantiated in `PRD.md` — outputs are specified to be role-aware, decision-focused, and length-capped so a Board/Steering Group member can act without extra interpretation; matches `agent-specifications.md`. Confirmed with TiesL. |
